...
$("#cart-dropdown").empty();
  ...
  $(".custom-qty input").on('input', function() {
    var customqty = parseInt($(this).val()); 
    var cartprice = parseFloat($(this).closest('li').find('.cart-price').data("value")); 
    var totalPrice = customqty * cartprice;
    $(this).closest('li').find('.cart-price').text('$' + totalPrice.toFixed(2));
    var cartSubtotal = parseFloat($("#cart-subtotal").data("value"));
...
$("#passwod_change").click(function (event) {
  ...
  // shopping cart cal
    ...
    function calculateSubtotal() {
      ...
      //  const subprice= parseFloat($(".subprice").text());
      const total_fullcharge = shipping_charge + subtotal;
      $("#total_fullcharge_display").text("Total:"+" ₹" + total_fullcharge);
        console.log(subtotal);
      });
